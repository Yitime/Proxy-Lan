...
// Display a profile name (or "name => name" result chain) in Chinese where known
export const displayProfileName = (name) => {
  if (!name) return name;
  return String(name).split(' => ').map((part) => profileNameMap[part] || part).join(' => ');
}
